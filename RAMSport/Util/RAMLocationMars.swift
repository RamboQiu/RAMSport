//
//  RAMLocationMars.swift
//  RAMSport
//
//  Created by rambo on 2020/2/18.
...
public extension CLLocation {
    ...
    class func transform_earth_from_mars(lat: Double, lng: Double) -> (Double, Double) {
    ...
    class func transform_earth_from_mars_lat(_ x: Double,_ y: Double) -> Double {
    ...
    class func transform_earth_from_mars_lng(_ x: Double,_ y: Double) -> Double {
    ...
    class func transform_mars_from_baidu(lat: Double, lng: Double) -> (Double,Double) {
        let x = lng , y = lat
        let z = sqrt(x * x + y * y) + 0.00002 * sin(y * x_pi)
        let theta = atan2(y, x) + 0.000003 * cos(x * x_pi)
        return (z * sin(theta) + 0.006, z * cos(theta) + 0.0065)
    }
    class func transform_baidu_from_mars(lat: Double, lng: Double) -> (Double,Double) {
        let x = lng - 0.0065 , y = lat - 0.006
        let z =  sqrt(x * x + y * y) - 0.00002 * sin(y * x_pi)
        let theta = atan2(y, x) - 0.000003 * cos(x * x_pi)
        return (z * sin(theta), z * cos(theta))
    }
    class func transform_sino_out_china(lat: Double, lng: Double) -> Bool {
